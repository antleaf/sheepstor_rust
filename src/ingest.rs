                                    ...
                                    match file.write_all(&body) {
                                        Ok(_) => {
                                            ...
                                            match website.update_sources() {
                                                Ok(_) => {
                                                    log::debug!("Successfully updated sources for website: {} before attempting to push changes", website.id);
                                                }
                                                Err(e) => {
                                                    log::error!("Failed to update sources for website: {} before attempting to push changes - {}", website.id, e);
                                                    return (StatusCode::INTERNAL_SERVER_ERROR, "Failed to update sources").into_response();
                                                }
                                            }
